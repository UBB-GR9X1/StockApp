    ...
    internal class BaseStocksRepository : IBaseStocksRepository
    {
        ...
        public void AddStock(BaseStock stock, int initialPrice = 100)
        {
            ...
            try
            {
                ...
                // Insert stock record
                const string insertStock = "INSERT INTO STOCK (STOCK_NAME, STOCK_SYMBOL, AUTHOR_CNP) VALUES (@StockName, @StockSymbol, @AuthorCNP)";
                ExecuteSql(insertStock, cmd =>
                {
                    cmd.Parameters.AddWithValue("@StockName", stock.Name);
                ...
                const string insertValue = "INSERT INTO STOCK_VALUE (STOCK_NAME, PRICE) VALUES (@StockName, @Price)";
                ExecuteSql(insertValue, cmd =>
                {
                    cmd.Parameters.AddWithValue("@StockName", stock.Name);
