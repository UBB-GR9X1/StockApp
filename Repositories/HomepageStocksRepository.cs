...
namespace StockApp.Repositories
{
    ...
    internal class HomepageStocksRepository : IHomepageStocksRepository
    {
        ...
        {
            // TODO: Use actual user authentication instead of hardcoded CNP
            ...
            {
                // FIXME: ExecuteNonQuery not needed before reading data
                command.ExecuteNonQuery();
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        this.UserCNP = reader["CNP"].ToString()
